...
<body>
    ...
    <div class="container">
        <div class="UbahKelas" style="color: black; font-size: 25px; font-family: Roboto; font-weight: 700; word-wrap: break-word; padding-top: 50px">
            Ubah Kelas</div>
    </div>

    <div class="container">
        @foreach($kelasItems as $kelasItem)
        <form method="POST" action="{{ route('ubah-kelas', ['idkelas' => $kelasItem->idkelas]) }}">
            @csrf
            @method('PATCH')
            <label for="kelas" style="color: black; font-size: 20px; font-family: Roboto; font-weight: 400; padding-top: 10px;">Kelas</label>
            <div class="form-row">
                <div class="form-group col-md-4">
                    <input type="text" class="form-control" id="kelas" name="kelas" placeholder="Masukkan Kelas" autocomplete="off" value="{{ $kelasItem->kelas }}">
                </div>
            </div>
